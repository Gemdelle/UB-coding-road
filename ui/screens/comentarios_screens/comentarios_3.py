...
def draw(frame, change_screen):
    ...
    task_label = WhiteStormLabel(task_frame,bg=task_frame.cget('bg'),font_size=13, width=60, height=5, text='❧ La partitura no se está mostrando en la hoja.\nDescomentar la variable partitura para mostrar el contenido.')
...
def correct_music_sheet(output_frame, code_frame, change_screen, input_area):
    ...
    next_level_button = tk.Button(code_frame, width=7, height=2, text="Next",
                                  command=lambda: (change_screen(Screens.COMENTARIOS_4), play_button_sound()))
