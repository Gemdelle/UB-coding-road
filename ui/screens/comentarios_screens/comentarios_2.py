...
def draw(frame, change_screen):
    ...
    user_completed_stage = user_progress["comentarios"]["current"] > 2
    ...
    subtitle_label = WhiteStormLabel(title_frame, text=f'0.3 Comentar una variable con """', font_size=16,
    ...
    task_label = WhiteStormLabel(task_frame,bg=task_frame.cget('bg'),font_size=13, width=60, height=5, text='❧ Apareció una mancha en la partitura,\n comentarla con comillas triples “”” “”” para que desaparezca.')
    ...
    if user_completed_stage:
        ...
        input_area.insert("1.0", 'mancha = True\nprint(mancha)')
...
def incorrect_music_sheet(output_frame):
    music_sheet_image = ClickableImage(output_frame, image_path=resource_path("assets\\images\\ex-1\\3.png"),
...
def correct_music_sheet(output_frame, code_frame, change_screen, input_area):
    ...
    next_level_button = tk.Button(code_frame, width=7, height=2, text="Next",
                                  command=lambda: (change_screen(Screens.COMENTARIOS_3), play_button_sound()))
