...
def draw(frame, change_screen):
    ...
    user_completed_stage = user_progress["comentarios"]["current"] > 1
    ...
    task_label = WhiteStormLabel(task_frame,bg=task_frame.cget('bg'),font_size=13, width=60, height=5, text='❧ Consigna: Descomenta la variable "titulo" que está actualmente\n comentada con el símbolo # para que se tenga en cuenta en la ejecución\n del programa.')
    ...
    if user_completed_stage:
        input_area.insert("1.0", 'titulo = "Sonata para piano No. 14"\nprint(titulo)')
    else:
        input_area.insert("1.0", '#titulo = "Sonata para piano No. 14"\n#print(titulo)')
...
def correct_music_sheet(output_frame, code_frame, change_screen, input_area):
    ...
    next_level_button = tk.Button(code_frame, width=7, height=2, text="Next",
                                  command=lambda: change_screen(Screens.COMENTARIOS_2))
