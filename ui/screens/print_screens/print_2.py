...
def draw(frame, change_screen):
    ...
    user_completed_stage = user_progress["print"]["current"] > 2
    ...
    subtitle_label = WhiteStormLabel(title_frame, text=f'1.3 Múltiples líneas  → print(word)', font_size=16,
    ...
    book_image = ClickableImage(title_frame, image_path=resource_path("assets\\images\\books\\2.png"), bg=frame.cget('bg'), image_size=(60, 80))
    book_image.grid(row=0, column=user_progress["print"]["total"] + 3, sticky='w', padx=(400, 0), pady=(0, 0))
    ...
    back_arrow_image.grid(row=0, column=user_progress["print"]["total"] + 3, sticky='w', padx=(470, 0), pady=(20, 0))
    ...
    task_label = WhiteStormLabel(task_frame,bg=task_frame.cget('bg'),font_size=13, width=60, height=5, text='❧ Consigna: Imprimir la frase “Climb the tower”\nubicando una palabra debajo de la otra.')
...
def correct_music_sheet(output_frame, code_frame, change_screen, input_area):
    music_sheet_image = ClickableImage(output_frame, image_path=resource_path("assets\\images\\ex-2\\2.png"),
                                       image_size=(285, 425), bg=output_frame.cget('bg'))
    music_sheet_image.grid(row=0, column=0, sticky='w', padx=(180, 0), pady=(20, 0))
    next_level_button = tk.Button(code_frame, width=7, height=2, text="Next",
                                  command=lambda: change_screen(Screens.PRINT_3))
