...
def draw(frame, change_screen):
    ...
    back_arrow_image = ClickableImage(title_frame, image_path=resource_path("assets\\images\\back_arrow.png"),
                                      ...
                                      bg=frame.cget('bg'))
    back_arrow_image.grid(row=0, column=user_progress["print"]["total"] + 3, sticky='w', padx=(150, 0),
                          pady=(5, 0))
    ...
    book_image.grid(row=0, column=user_progress["print"]["total"] + 3, sticky='w', padx=(270, 0), pady=(0, 0))
...
def correct_music_sheet(output_frame, code_frame, change_screen, input_area):
    ...
    next_level_button = tk.Button(code_frame, width=7, height=2, text="Next",
                                  command=lambda: (change_screen(Screens.PRINT_5), play_button_sound()))
