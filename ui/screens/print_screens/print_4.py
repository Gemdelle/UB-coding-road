...
def draw(frame, change_screen):
    ...
    user_completed_stage = user_progress["print"]["current"] > 4
    ...
    subtitle_label = WhiteStormLabel(title_frame, text=f'1.4. Operador + → print(word + word)', font_size=16,
    ...
    task_label = WhiteStormLabel(task_frame,bg=task_frame.cget('bg'),font_size=13, width=60, height=5, text='❧ Consigna: Imprimir nuevamente “Climb the tower”,\npero esta vez a modo de frase y\nutilizando el operador de concatenación (+).')
...
def correct_music_sheet(output_frame, code_frame, change_screen, input_area):
    ...
    next_level_button = tk.Button(code_frame, width=7, height=2, text="Next",
                                  command=lambda: change_screen(Screens.PRINT_5))
