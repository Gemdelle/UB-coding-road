...
def draw(frame, change_screen):
    ...
    user_completed_stage = user_progress["print"]["current"] > 0
    ...
    subtitle_label = WhiteStormLabel(title_frame, text=f'1.1. Texto → print("Tower")', font_size=16,
    ...
    task_label = WhiteStormLabel(task_frame,bg=task_frame.cget('bg'),font_size=13, width=60, height=5, text='❧ Consigna: Imprimir la palabra tower en el espejo.')
    ...
    if user_completed_stage:
        input_area.insert("1.0", 'print(“tower”)')
    else:
...
def correct_music_sheet(output_frame, code_frame, change_screen, input_area):
    ...
    next_level_button = tk.Button(code_frame, width=7, height=2, text="Next",
                                  command=lambda: change_screen(Screens.PRINT_1))
