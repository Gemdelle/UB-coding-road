...
def process_input(input_area, process_button, code_frame, correct, incorrect):
    ...
    input_text_with_validation = input_text + 'print(isinstance(numero, float))'
...
def draw(frame, change_screen):
    ...
    user_completed_stage = user_progress["transformacion"]["current"] > 1
    ...
    subtitle_label = WhiteStormLabel(title_frame, text=f'3.2 str a float', font_size=16,
    ...
    if user_completed_stage:
        input_area.insert("1.0", 'numero = float("26.3")')
    else:
        input_area.insert("1.0", 'numero = "26.3"\n')
...
def incorrect_music_sheet(output_frame):
    music_sheet_image = ClickableImage(output_frame, image_path=resource_path("assets\\images\\ex-4\\4-2-str.png"),
...
def correct_music_sheet(output_frame, code_frame, change_screen, input_area):
    music_sheet_image = ClickableImage(output_frame, image_path=resource_path("assets\\images\\ex-4\\4-2-float.png"),
                                       image_size=(598, 412), bg=output_frame.cget('bg'))
    music_sheet_image.grid(row=0, column=0, sticky='w', padx=(0, 0), pady=(20, 0))
    next_level_button = tk.Button(code_frame, width=7, height=2, text="Next",
                                  command=lambda: (change_screen(Screens.TRANSFORMACION_2), play_button_sound()))
