...
def draw(frame, change_screen):
    ...
    user_completed_stage = user_progress["input"]["current"] > 2
    ...
    subtitle_label = WhiteStormLabel(title_frame, text=f'4.3 Ingresar un número en str y transformarlo después a float',
                                     font_size=16,
    ...
    back_arrow_image = ClickableImage(title_frame, image_path=resource_path("assets\\images\\back_arrow.png"),
                                      ...
                                      bg=frame.cget('bg'))
    back_arrow_image.grid(row=0, column=user_progress["input"]["total"] + 3, sticky='w', padx=(200, 0),
                          pady=(5, 0))
    ...
    book_image.grid(row=0, column=user_progress["input"]["total"] + 3, sticky='w', padx=(320, 0), pady=(0, 0))
...
def correct_music_sheet(output_frame, code_frame, change_screen, input_area):
    music_sheet_image = ClickableImage(output_frame, image_path=resource_path("assets\\images\\ex-n-4\\4-4.png"),
                                       image_size=(598, 412), bg=output_frame.cget('bg'))
    music_sheet_image.grid(row=0, column=0, sticky='w', padx=(0, 0), pady=(20, 0))
