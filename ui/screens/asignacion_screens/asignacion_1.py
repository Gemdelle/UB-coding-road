...
def process_input(input_area, process_button, correct, incorrect):
    ...
    input_text_with_validation = input_text + 'print(planeta1 == "Earth" and planeta2 == "Moon"and distancia == 384400)'
...
def draw(frame, change_screen):
    ...
    user_completed_stage = user_progress["asignacion"]["current"] > 1
    ...
    subtitle_label = WhiteStormLabel(title_frame, text=f'2.2 Int', font_size=16,
    ...
    task_label = WhiteStormLabel(task_frame,bg=task_frame.cget('bg'),font_size=13, width=60, height=5, text='❧ Consigna: Declarar una variable <distancia> de tipo entero\n con la distancia entre la Tierra y la Luna (384400 km.).')
...
def correct_music_sheet(output_frame, code_frame, change_screen, input_area):
    ...
    next_level_button = tk.Button(code_frame, width=7, height=2, text="Next",
                                  command=lambda: change_screen(Screens.ASIGNACION_2))
