...
def draw(frame, change_screen):
    ...
    user_completed_stage = user_progress["asignacion"]["current"] > 5
    ...
    subtitle_label = WhiteStormLabel(title_frame, text=f'2.6 Sobreescribir variables de tipo int', font_size=16,
    ...
    task_label = WhiteStormLabel(task_frame,bg=task_frame.cget('bg'),font_size=13, width=60, height=5, text='❧ Consigna: Sobreescribir la variable <distancia> para que guarde\nel valor 77 (millones de km.)')
    ...
    if user_completed_stage:
        input_area.insert("1.0", 'venus = "Venus"\nmercurio = "Mercury"\ndistancia = 77')
    else:
        input_area.insert("1.0", 'venus = "Venus"\nmercurio = "Mercury"\ndistancia = 12\n')
...
def incorrect_music_sheet(output_frame):
    music_sheet_image = ClickableImage(output_frame, image_path=resource_path("assets\\images\\ex-3\\02-06.png"),
...
def correct_music_sheet(output_frame, code_frame, change_screen, input_area):
    music_sheet_image = ClickableImage(output_frame, image_path=resource_path("assets\\images\\ex-3\\02-05.png"),
                                       image_size=(598, 412), bg=output_frame.cget('bg'))
    music_sheet_image.grid(row=0, column=0, sticky='w', padx=(0, 0), pady=(20, 0))
