...
def draw(frame, change_screen):
    ...
    user_completed_stage = user_progress["asignacion"]["current"] > 4
    ...
    subtitle_label = WhiteStormLabel(title_frame, text=f'2.5 Sobreescribir variables de tipo string', font_size=16,
    ...
    if user_completed_stage:
        input_area.insert("1.0", 'planeta1 = "Venus"\nplaneta2 = "Mercury"')
    else:
...
def correct_music_sheet(output_frame, code_frame, change_screen, input_area):
    ...
    next_level_button = tk.Button(code_frame, width=7, height=2, text="Next",
                                  command=lambda: change_screen(Screens.ASIGNACION_5))
