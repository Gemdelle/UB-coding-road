...
def draw(frame, change_screen):
    ...
    user_completed_stage = user_progress["asignacion"]["current"] > 0
    ...
    subtitle_label = WhiteStormLabel(title_frame, text=f'2.1 String', font_size=16,
    ...
    book_image = ClickableImage(title_frame, image_path=resource_path("assets\\images\\books\\3.png"), bg=frame.cget('bg'), image_size=(60, 80))
    book_image.grid(row=0, column=user_progress["asignacion"]["total"] + 3, sticky='w', padx=(470, 0), pady=(0, 0))

    back_arrow_image = ClickableImage(title_frame, image_path=resource_path("assets\\images\\back_arrow.png"),image_size=(87, 46), callback=lambda: change_screen(Screens.LANDING), bg=frame.cget('bg'))
    back_arrow_image.grid(row=0, column=user_progress["asignacion"]["total"] + 3, sticky='w', padx=(400, 0), pady=(20, 0))
    ...
    task_label = WhiteStormLabel(task_frame,bg=task_frame.cget('bg'),font_size=13, width=60, height=5, text='❧ Consigna: Declarar las variables <planeta1> y <planeta2>\npara la Tierra y la Luna (valores Earth, Moon).')
...
def incorrect_music_sheet(output_frame):
    another_empty_frame = tk.Frame(output_frame, width=1, height=450, bg=output_frame.cget('bg'))
    another_empty_frame.grid(row=1, column=0, sticky='e', padx=(0, 0), pady=(0, 0))

def correct_music_sheet(output_frame, code_frame, change_screen, input_area):
    music_sheet_image = ClickableImage(output_frame, image_path=resource_path("assets\\images\\ex-3\\02-01.png"),
    ...
    next_level_button = tk.Button(code_frame, width=7, height=2, text="Next",
                                  command=lambda: change_screen(Screens.ASIGNACION_1))
