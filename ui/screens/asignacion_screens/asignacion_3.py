...
def process_input(input_area, process_button, correct, incorrect):
    ...
    input_text_with_validation = input_text + 'print(estrella == "Sun" and luz == True)'
...
def draw(frame, change_screen):
    ...
    user_completed_stage = user_progress["asignacion"]["current"] > 3
    ...
    subtitle_label = WhiteStormLabel(title_frame, text=f'2.4 Boolean', font_size=16,
    ...
    book_image.grid(row=0, column=user_progress["asignacion"]["total"] + 3, sticky='w', padx=(400, 0), pady=(0, 0))
    ...
    back_arrow_image.grid(row=0, column=user_progress["asignacion"]["total"] + 3, sticky='w', padx=(470, 0), pady=(20, 0))
    ...
    task_label = WhiteStormLabel(task_frame,bg=task_frame.cget('bg'),font_size=13, width=60, height=5, text='❧ Consigna: Declarar una variable <luz>\nde tipo boolean para encender el sol.')
    ...
    if user_completed_stage:
        input_area.insert("1.0", 'estrella = "Sun"\nluz = True')
    else:
        input_area.insert("1.0", 'estrella = "Sun"')
...
def incorrect_music_sheet(output_frame):
    music_sheet_image = ClickableImage(output_frame, image_path=resource_path("assets\\images\\ex-3\\02-04-no-light.png"),
...
def correct_music_sheet(output_frame, code_frame, change_screen, input_area):
    music_sheet_image = ClickableImage(output_frame, image_path=resource_path("assets\\images\\ex-3\\02-04-light.png"),
                                       image_size=(598, 412), bg=output_frame.cget('bg'))
    music_sheet_image.grid(row=0, column=0, sticky='w', padx=(0, 0), pady=(20, 0))
    next_level_button = tk.Button(code_frame, width=7, height=2, text="Next",
                                  command=lambda: change_screen(Screens.ASIGNACION_4))
